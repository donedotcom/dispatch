...
var Router = function (resources) {
  ...
  this.loadingQueue = []; // Loading queue for out-of-order controllers
...
Router.prototype.DefaultActionsSingular = [
  { name: 'index', method: 'GET', scope: 'collection', parameterMatch: true },
];
...
/**
  ...
  If root is true, this resource exists at /.  There can only be one root resource, and any parent value is ignored.  The root is last on the route chain to avoid conflicts.
  @returns Undefined
*/
Router.prototype.addResource = function (resource) {
  ...
  if (this._findRoute(resource.name)) {
    logger.get().warn('Router: routes already exist for resource; may overwrite', resource);
  }

  if (resource.parent) {
    parentRoute = this._findRoute(resource.parent);
    if (!parentRoute) {
      logger.get().debug('Parent resource not found: queueing to see if parent is loaded later', resource);
      this.loadingQueue.push(resource);
      return;
    }
    // Children get inserted before their parents to make sure the routes are found properly
    insertIndex = this.routes.indexOf(parentRoute);
    route = parentRoute.route.slice(0, -1); // Remove the '$'
    logger.get().debug('Found parent route ' + route + ' at index ' + insertIndex);
  }
  ...
  this.actions[resource.name] = resource.customActions.concat(resource.singular ?
    this.DefaultActionsSingular : this.DefaultActions);

  routeObject = { name: resource.name, route : route, parent : parentRoute, root : resource.root };

  // Root always goes at the end to prevent it from overtaking "child" routes
  // eg, if '/' is the forums controller then '/reviews' should be the reviews controller,
  // not the 'reviews' action on the forums controller.  Obviously this will lead to
  // other conflicts, but this approach is predictable instead of dependent on load-order.
  if (resource.root) {
  ...
  logger.get().debug('Adding new route ' + route, { actions : _.map(this.actions[resource.name], function (action) {
    return action.name + '(' + action.method + ', ' + action.scope + ')';
  }) });
  
  resource.on('error', function (err, req, res) {
    emit('error', err, req, res);
  });

  if (!_.isEmpty(this.loadingQueue)) {
    this._tryLoadingQueue();
  }
};

Router.prototype._tryLoadingQueue = function () {
  var self = this,
      queueCopy = _.clone(this.loadingQueue),
      loaded = 0;

  // Try to load any resources in the queue whose parents we can now find
  _.each(queueCopy, function (resource) {
    var parentRoute = self._findRoute(resource.parent);
    if (parentRoute) {
      logger.get().debug('Now ready to load "' + resource.name + '"');
      
      // Update loading queue so we don't get recursively called by addResource
      self.loadingQueue = _.reject(self.loadingQueue, function (r1) { return r1 === resource; });
  ...
  if (loaded !== 0) {
    // If we loaded any resources, try again
    self._tryLoadingQueue();
  }
};
...
Router.prototype.middleware = function (req, res, next) {
  ...
  logger.get().debug('Router URL ' + url + ' (' + req.method + ')');
  ...
  if (!_.isEmpty(matches)) {
    ...
    url = url.replace(/\.\w+$/, '');
    logger.get().debug('Detected format: ' + req.format);
  }

  resource = _.detect(this.routes, function (route) {
    var regexp = route.route;

    logger.get().debug('Testing ' + regexp);
    matches = url.match(regexp);

    if (!_.isEmpty(matches)) {
      matches = _.compact(matches).slice(1);
      logger.get().debug('Found resource ' + route.name, matches);
      return route.name;
    }
  }.bind(this));
  ...
  if (resource) {
    ...
    if (action) {
      logger.get().debug('Direct match', action);
      matches.pop();
      ...
      action = _.detect(_.filter(actions, function (action) {
        ...
        if (this._checkRequestAction(resource, action, matches)) {
          logger.get().debug('Parameter match', action);
          return action;
        }
      }.bind(this));
    }
    ...
    if (!action) {
      logger.get().info('No available action found for ' + url);
      next();
    } else {
      logger.get().info('Found action', action);
      ...
      while (obj && !_.isEmpty(matches)) {
        ...
        logger.get().debug('Adding parent ID to request: ' + parentId + ' = ' + _.last(matches));
...
Router.prototype._checkRequestAction = function (resource, action, matches) {
  ...
  logger.get().debug('Testing action (param count: ' + paramCount + ')', action);
...
Router.prototype._createRouteHelper = function (resource, route) {
  ...
  if (!self.path) {
    var path = {};
    path.toString = function (pathVariable) {
      var res;

      pathVariable = pathVariable || 'path';

      res = 'var ' + pathVariable + ' = {\n';
  ...
  logger.get().debug('Create route helper', { route : route });
  ...
  logger.get().debug('Path helpers', { paths : _.keys(self.path) });
};
