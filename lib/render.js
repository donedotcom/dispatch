...
Render.prototype.render = function (req, res, next, options) {
  ...
  function _sendBody(body, headers) {
    headers = headers || [];
    try {
      ...
      // Deal with the possibility that we have already sent the headers
      logger.get().error(err.stack, { url : req.url, action : req.action });
    }
  }
  ...
  if (options.json) {
    ...
    _sendBody(body, [['Content-Type', 'application/json']]);
...
Render.prototype._fileNames = function (fileName) {
  ...
  // By default add the name; tried in order so this will always be last
  names.push(fileName);
