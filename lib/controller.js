...
var Controller = module.exports = function (options) {
  ...
  this.queue = [];
  this.running = false;
  this.verboseErrorLogging = false;
  ...
  var defaultHelpers = {
    render: function (req, res) {
      return function (options) {
        self.renderHelper.render(req, res, res.next, options || {});
      };
    },
  ...
  if (!this.name) {
    this.logger.get().warn('No name for controller', params);
  }
}
...
Controller.prototype._runJob = function (job) {
  ...
  try {
    this._invokeSerial(_.compact(beforeHooks), function (hook, callback) {
      ...
      if (!err) {
        self[req.action].call(this, req, res, res.next);
      }
    });
  } catch (err) {
    this.logger.get().error(err.message);
    this.logger.get().error(err.stack);
    res.statusCode = 500;
